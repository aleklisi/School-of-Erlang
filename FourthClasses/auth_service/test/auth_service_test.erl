...
add_user(Login, Password, InputState) ->
    {_, _Reply, NewState} = user_exists:handle_call({add, Login, Password}, ignored_from, InputState),
    NewState.

authenticate_user(Login, Password, InputState) ->
    {_, _Reply, NewState} = user_exists:handle_call({authenticate, Login, Password}, ignored_from, InputState),
    NewState.

user_exists(Login, State) ->
    maps:is_key(Login, State).