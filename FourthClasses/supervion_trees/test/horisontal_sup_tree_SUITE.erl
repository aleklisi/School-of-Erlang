-module(horisontal_sup_tree_SUITE).
...
horisontal_tree(_Config) ->
    TopSupChildren = supervisor:which_children(top_sup),
    ct:log("TopSupChildren = ~p\n", [TopSupChildren]),
    ?assertEqual(3, length(TopSupChildren)),

    ASupChildren = supervisor:which_children(a_sup),
    ?assertEqual(0, length(ASupChildren)),

    BSupChildren = supervisor:which_children(b_sup),
    ?assertEqual(0, length(BSupChildren)),
